...
# --- add these argparse options (defaults keep behavior identical = no front-end) ---
...
parser.add_argument('--sym', action='store_true',
                    help='enable Hartley–cosine symmetric front-end (default: disabled)')
parser.add_argument('--sym_kernel', type=int, default=9,
                    help='front-end kernel size (odd, default: 9)')
parser.add_argument('--sym_h', type=float, default=1.0,
                    help='h scaling in the Hartley–cosine formula (default: 1.0)')
parser.add_argument('--sym_causal', action='store_true',
                    help='make the front-end causal (left-padded) (default: False)')
parser.add_argument('--sym_no_residual', action='store_true',
                    help='use front-end output only (no residual add)')
...
model = TCN(args.emsize, n_characters, num_chans,
            kernel_size=k_size, dropout=dropout, emb_dropout=emb_dropout)

# 2) If requested, prepend your Hartley–cosine symmetric front-end to the internal TCN
if args.sym:
    from TCN.common.symmetric_conv1d import SymmetricConv1d
    import torch.nn as nn

    # small residual wrapper (so you can do x + front(x))
    class _Residual(nn.Module):
        def __init__(self, mod): super().__init__(); self.mod = mod
        def forward(self, x): return x + self.mod(x)

    # channels = embedding dim (emsize); keep in==out so residual add is valid
    _front = SymmetricConv1d(
        in_channels=args.emsize,
        out_channels=args.emsize,
        kernel_size=args.sym_kernel,
        h=args.sym_h,
        causal=args.sym_causal,
        bias=False
    )

    # put it right before the TemporalConvNet inside the model
    if hasattr(model, "tcn"):
        model.tcn = nn.Sequential(
            _Residual(_front) if not args.sym_no_residual else _front,
            model.tcn
        )
    else:
        raise AttributeError("Expected model.tcn to exist; update attribute name if different.")
